...
(function($) {
    ...
    window.PeanutAdmin = {
        ...
        initDarkMode: function() {
            ...
            var savedTheme = localStorage.getItem('peanut_theme');
            var prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
            var isDark = savedTheme === 'dark' || (!savedTheme && prefersDark);

            if (isDark) {
                $('body').addClass('peanut-dark-mode');
            }

            // Create toggle button if header actions exist
            if ($('.peanut-header-actions').length && !$('#peanut-theme-toggle').length) {
                var $toggle = $('<button type="button" id="peanut-theme-toggle" class="button peanut-theme-toggle" title="Toggle dark mode">' +
                    '<span class="dashicons dashicons-' + (isDark ? 'sun' : 'moon') + '"></span>' +
                    '</button>');
                $('.peanut-header-actions').prepend($toggle);
            }

            // Toggle event
            $(document).on('click', '#peanut-theme-toggle, .peanut-theme-toggle', function(e) {
                e.preventDefault();
                self.toggleDarkMode();
            });
            ...
            window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', function(e) {
                if (!localStorage.getItem('peanut_theme')) {
                    if (e.matches) {
                        $('body').addClass('peanut-dark-mode');
                    } else {
                        $('body').removeClass('peanut-dark-mode');
                    }
        ...
        toggleDarkMode: function() {
            var isDark = $('body').toggleClass('peanut-dark-mode').hasClass('peanut-dark-mode');
            localStorage.setItem('peanut_theme', isDark ? 'dark' : 'light');
            this.updateThemeIcon();

            // Show feedback
            this.showNotice('success', isDark ? 'Dark mode enabled' : 'Light mode enabled');
        },
        ...
        updateThemeIcon: function() {
            var isDark = $('body').hasClass('peanut-dark-mode');
            $('#peanut-theme-toggle .dashicons, .peanut-theme-toggle .dashicons')
                .removeClass('dashicons-sun dashicons-moon')
                .addClass(isDark ? 'dashicons-sun' : 'dashicons-moon');
        },

        /**
         * Initialize Sidebar Toggle (Collapsible)
         * @since 2.4.0
         */
        initSidebarToggle: function() {
            ...
            var isCollapsed = localStorage.getItem('peanut_sidebar_collapsed') === 'true';

            if (isCollapsed) {
                $('#adminmenuwrap, #adminmenu').addClass('peanut-sidebar-collapsed');
                $('body').addClass('peanut-sidebar-collapsed');
            }

            // Create toggle button if not exists
            if (!$('#peanut-sidebar-toggle').length) {
                var $toggle = $('<button type="button" id="peanut-sidebar-toggle" class="peanut-sidebar-toggle" title="Toggle sidebar">' +
                    '<span class="dashicons dashicons-arrow-' + (isCollapsed ? 'right' : 'left') + '-alt2"></span>' +
                    '</button>');
                $('#adminmenuwrap').append($toggle);
            }

            // Toggle event
            $(document).on('click', '#peanut-sidebar-toggle', function(e) {
                e.preventDefault();
                self.toggleSidebar();
            });
        },
        ...
        toggleSidebar: function() {
            var isCollapsed = $('body').toggleClass('peanut-sidebar-collapsed').hasClass('peanut-sidebar-collapsed');
            $('#adminmenuwrap, #adminmenu').toggleClass('peanut-sidebar-collapsed');
            localStorage.setItem('peanut_sidebar_collapsed', isCollapsed);

            // Update icon
            $('#peanut-sidebar-toggle .dashicons')
                .removeClass('dashicons-arrow-left-alt2 dashicons-arrow-right-alt2')
                .addClass(isCollapsed ? 'dashicons-arrow-right-alt2' : 'dashicons-arrow-left-alt2');
        },
        ...
        initKeyboardShortcuts: function() {
            ...
            $(document).on('keydown', function(e) {
                ...
                var isInput = $(e.target).is('input, textarea, select, [contenteditable]');

                // Ctrl/Cmd + S = Save
                if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                    if (!isInput || $(e.target).closest('form').length) {
                        e.preventDefault();
                        var $form = $('.peanut-form, form.peanut-settings-form').first();
                        if ($form.length) {
                            $form.find('[type="submit"]').first().click();
                            self.showNotice('info', 'Saving...');
                        }
                    }
                }
                ...
                if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
                    e.preventDefault();
                    var $search = $('input[name="s"], .peanut-search-input, #post-search-input').first();
                    if ($search.length) {
                        $search.focus().select();
                    }
                }

                // Escape = Close modals (already handled, but ensure it works)
                if (e.key === 'Escape') {
                    self.closeAllModals();
                }
            });
